...
/**
 * REST controller for managing {@link com.sa.sa.domain.Bo2mCar}.
 */
@RestController
@RequestMapping("/api")
public class Bo2mCarResource {
    ...
    private static final String ENTITY_NAME = "bo2mCar";
    ...
    /**
     * {@code SEARCH  /_search/bo-2-m-cars?query=:query} : search for the bo2mCar corresponding
     * to the query.
     *
     * @param query the query of the bo2mCar search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/bo-2-m-cars")
    public ResponseEntity<List<Bo2mCar>> searchBo2mCars(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Bo2mCars for query {}", query);
        Page<Bo2mCar> page = bo2mCarService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
