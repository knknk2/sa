...
/**
 * REST controller for managing {@link com.sa.sa.domain.Bo2mCarDTO}.
 */
@RestController
@RequestMapping("/api")
public class Bo2mCarDTOResource {
    ...
    private static final String ENTITY_NAME = "bo2mCarDTO";
    ...
    public ResponseEntity<Bo2mCarDTO> updateBo2mCarDTO(@Valid @RequestBody Bo2mCarDTO bo2mCarDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, bo2mCarDTO.getId().toString()))
            .body(result);
    }
    ...
    /**
     * {@code SEARCH  /_search/bo-2-m-car-dtos?query=:query} : search for the bo2mCarDTO corresponding
     * to the query.
     *
     * @param query the query of the bo2mCarDTO search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/bo-2-m-car-dtos")
    public ResponseEntity<List<Bo2mCarDTO>> searchBo2mCarDTOS(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Bo2mCarDTOS for query {}", query);
        Page<Bo2mCarDTO> page = bo2mCarDTOService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
