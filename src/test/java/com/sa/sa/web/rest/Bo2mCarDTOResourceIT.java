...
import com.sa.sa.repository.Bo2mCarDTORepository;
import com.sa.sa.repository.search.Bo2mCarDTOSearchRepository;
import com.sa.sa.service.Bo2mCarDTOService;
...
public class Bo2mCarDTOResourceIT {
     ...
     *
     * @see com.sa.sa.repository.search.Bo2mCarDTOSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createBo2mCarDTO() throws Exception {
        ...
        // Validate the Bo2mCarDTO in Elasticsearch
        verify(mockBo2mCarDTOSearchRepository, times(1)).save(testBo2mCarDTO);
    }
    ...
    public void createBo2mCarDTOWithExistingId() throws Exception {
        ...
        // Validate the Bo2mCarDTO in Elasticsearch
        verify(mockBo2mCarDTOSearchRepository, times(0)).save(bo2mCarDTO);
    }
    ...
    public void updateBo2mCarDTO() throws Exception {
        ...
        reset(mockBo2mCarDTOSearchRepository);
    ...
    public void deleteBo2mCarDTO() throws Exception {
        ...
        // Validate the Bo2mCarDTO in Elasticsearch
        verify(mockBo2mCarDTOSearchRepository, times(1)).deleteById(bo2mCarDTO.getId());
    }

    @Test
    @Transactional
    public void searchBo2mCarDTO() throws Exception {
        // Initialize the database
        bo2mCarDTOService.save(bo2mCarDTO);
        when(mockBo2mCarDTOSearchRepository.search(queryStringQuery("id:" + bo2mCarDTO.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(bo2mCarDTO), PageRequest.of(0, 1), 1));
        // Search the bo2mCarDTO
        restBo2mCarDTOMockMvc.perform(get("/api/_search/bo-2-m-car-dtos?query=id:" + bo2mCarDTO.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(bo2mCarDTO.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
