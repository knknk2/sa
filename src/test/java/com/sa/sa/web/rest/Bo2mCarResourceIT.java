...
import com.sa.sa.service.Bo2mCarService;
...
public class Bo2mCarResourceIT {
     ...
     *
     * @see com.sa.sa.repository.search.Bo2mCarSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createBo2mCar() throws Exception {
        ...
        // Validate the Bo2mCar in Elasticsearch
        verify(mockBo2mCarSearchRepository, times(1)).save(testBo2mCar);
    }
    ...
    public void createBo2mCarWithExistingId() throws Exception {
        ...
        // Validate the Bo2mCar in Elasticsearch
        verify(mockBo2mCarSearchRepository, times(0)).save(bo2mCar);
    }
    ...
    public void updateBo2mCar() throws Exception {
        ...
        reset(mockBo2mCarSearchRepository);
    ...
    public void deleteBo2mCar() throws Exception {
        ...
        // Validate the Bo2mCar in Elasticsearch
        verify(mockBo2mCarSearchRepository, times(1)).deleteById(bo2mCar.getId());
    }

    @Test
    @Transactional
    public void searchBo2mCar() throws Exception {
        // Initialize the database
        bo2mCarService.save(bo2mCar);
        when(mockBo2mCarSearchRepository.search(queryStringQuery("id:" + bo2mCar.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(bo2mCar), PageRequest.of(0, 1), 1));
        // Search the bo2mCar
        restBo2mCarMockMvc.perform(get("/api/_search/bo-2-m-cars?query=id:" + bo2mCar.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(bo2mCar.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
