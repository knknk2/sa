...
public class Bo2mOwnerResourceIT {
     ...
     *
     * @see com.sa.sa.repository.search.Bo2mOwnerSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createBo2mOwner() throws Exception {
        ...
        // Validate the Bo2mOwner in Elasticsearch
        verify(mockBo2mOwnerSearchRepository, times(1)).save(testBo2mOwner);
    }
    ...
    public void createBo2mOwnerWithExistingId() throws Exception {
        ...
        // Validate the Bo2mOwner in Elasticsearch
        verify(mockBo2mOwnerSearchRepository, times(0)).save(bo2mOwner);
    }
    ...
    public void updateBo2mOwner() throws Exception {
        ...
        reset(mockBo2mOwnerSearchRepository);
    ...
    public void deleteBo2mOwner() throws Exception {
        ...
        // Validate the Bo2mOwner in Elasticsearch
        verify(mockBo2mOwnerSearchRepository, times(1)).deleteById(bo2mOwner.getId());
    }

    @Test
    @Transactional
    public void searchBo2mOwner() throws Exception {
        // Initialize the database
        bo2mOwnerService.save(bo2mOwner);
        when(mockBo2mOwnerSearchRepository.search(queryStringQuery("id:" + bo2mOwner.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(bo2mOwner), PageRequest.of(0, 1), 1));
        // Search the bo2mOwner
        restBo2mOwnerMockMvc.perform(get("/api/_search/bo-2-m-owners?query=id:" + bo2mOwner.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(bo2mOwner.getId().intValue())))
